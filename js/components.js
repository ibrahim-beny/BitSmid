(function () {
  ...
  if (typeof AOS !== 'undefined') {
    AOS.init({
      once:     true,
      offset:   0,          // trigger zodra element in beeld komt
      duration: 500,        // vlotter: 500ms ipv 700ms
      easing:   'ease-out-cubic',
      delay:    0,
      disable:  false
    });

    // Fallback: na 800ms alle [data-aos] die in viewport zitten maar nog niet
    // geanimeerd zijn forceren we zichtbaar. Vangt snelle scrollers op.
    setTimeout(function () {
    ...
    // Extra fallback op scroll-einde: zet resterende elementen zichtbaar
    var aosScrollFallback = function () {
      var els = document.querySelectorAll('[data-aos]:not(.aos-animate)');
      if (!els.length) {
        window.removeEventListener('scroll', aosScrollFallback);
        return;
      }
      els.forEach(function (el) {
        var rect = el.getBoundingClientRect();
    ...
    window.addEventListener('scroll', aosScrollFallback, { passive: true });
  }
})();
